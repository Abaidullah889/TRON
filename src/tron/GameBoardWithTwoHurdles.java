...
/**
 * The GameBoardWithTwoHurdles class represents a game board with two large hurdles 
 * positioned at equal distances from the center of the screen. Players must navigate 
 * their motorcycles around these hurdles to avoid collisions while competing against 
 * each other.
 * 
 ...
 * @author 9
 */
public class GameBoardWithTwoHurdles extends JPanel {
    ...
    /**
     * The first hurdle located on the left side of the board.
     */
    private final Rectangle hurdle1 = new Rectangle(BOARD_WIDTH / 4 - 50, BOARD_HEIGHT / 2 - 50, 100, 100);

    /**
     * The second hurdle located on the right side of the board.
     */
    private final Rectangle hurdle2 = new Rectangle(3 * BOARD_WIDTH / 4 - 50, BOARD_HEIGHT / 2 - 50, 100, 100);

    /**
     * Constructs a new game board with two hurdles.
     * 
     ...
     */
    public GameBoardWithTwoHurdles(String player1Name, String player2Name, Color player1Color, Color player2Color, JLabel timeLabel, GameController controller) {
    ...
    private void checkCollisions() {
        ...
        // Check head-to-head collision (draw condition)
        ...
        // Check boundary collision for both players
        for (Motorcycle motorcycle : motorcycles) {
        ...
        // Check collision with either hurdle
        for (Motorcycle motorcycle : motorcycles) {
            if (hurdle1.contains(motorcycle.getX(), motorcycle.getY()) || hurdle2.contains(motorcycle.getX(), motorcycle.getY())) {
                gameTimer.stop();
                Motorcycle winner = (motorcycle == p1) ? p2 : p1;
                showGameOverDialog(motorcycle.getName() + " lost (Hit a Hurdle)!");
                controller.updateHighScores(winner.getName());
                return;
            }
        }
